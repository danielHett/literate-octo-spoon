import { buildTreeFromCounts, buildTreeFromPaths, buildEncodingDict, isLeaf } from "./utils";
let END_CHAR = '\0';
export const encode = (bytes) => {
    ...
    for (let byte of bytes) {
        if (counts.has(byte))
            counts.set(byte, counts.get(byte) + 1);
        else
            counts.set(byte, 1);
    }
    // Don't forget to set the null byte. 
    counts.set(null, 1);
    let root = buildTreeFromCounts(counts);
    // Now we need a dictionary. 
    let encodingDict = buildEncodingDict(root);
    // We need to know how many bytes are needed to encode all characters. 
    // We add extra bits for the null character. 
    const numBits = bytes.reduce((acc, byte) => acc + encodingDict.get(byte).length, 0) + encodingDict.get(null).length;
    const encodedMessage = new Uint8Array(Math.ceil(numBits / 8));
    // i tracks the current bit, j tracks the current byte. 
    let i = 7, j = 0;
    for (let byte of bytes) {
        // Get all bits in the encoding. 
        let bits = encodingDict.get(byte);
        for (let bit of bits) {
            // Do some work. 
    ...
    const stringifiedEncodingDict = new Uint8Array([...encodingDict].reduce((acc, [_, bits]) => acc + bits.length + 3, 0) + 3);
    i = 0;
    ...
    stringifiedEncodingDict[i++] = 120;
    // Merge and done. 
    let finalEncoding = new Uint8Array([...stringifiedEncodingDict, ...encodedMessage]);
    return finalEncoding;
};
export const decode = (fileBuffer) => {
    // First we need to read the encoding dictionary. 
    let encodingDict = new Map();
    let start = 0;
    let byte;
    let path = [];
    ...
    // Finally we decode. 
    let bytes = [];
    let root = buildTreeFromPaths(encodingDict);
    let curr = root;
    for (let i = start; i <= fileBuffer.length - 1; i++) {
        for (let j = 7; j >= 0; j--) {
            // Read a bit. 
            let bit = (fileBuffer[i] >> j) & 1;
